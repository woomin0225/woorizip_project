...
@router.post(
    "/room",
    summary="방 정보 RAG 검색",
    description="입력 텍스트를 사용해 방 벡터 검색을 수행합니다.",
)
...
@router.post(
    "/room/explanation",
    summary="방 정보 RAG 설명 생성",
    description="입력 텍스트를 사용해 Qdrant 검색 결과 기반 설명을 생성합니다.",
)
async def room_search_explanation(
    text: Annotated[str, Body(embed=False)],
    rag_service: Annotated[RagService, Depends(get_rag_service)],
):
    rag_result = await rag_service.room_rag_explanation(text)
