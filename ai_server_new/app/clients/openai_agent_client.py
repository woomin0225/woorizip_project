...
class OpenAIAgentClient:
        ...
        for url, payload in attempts:
            ...
            try:
                ...
                if exc.code not in (404, 405):
                    raise ValueError(
                        "Agent API 호출 실패: " + " | ".join(error_parts)
                    ) from exc
            except error.URLError as exc:
                raise ValueError(
                    f"Agent API 통신 중 오류가 발생했습니다: {exc.reason}"
                ) from exc
    ...
    def _build_attempts(
        ...
        is_exact_responses = "/protocols/openai/responses" in base or base.endswith(
            "/responses"
        )
    ...
    def _build_auth_headers(self) -> dict[str, str]:
        ...
        raise ValueError(
            f"지원하지 않는 AI_AGENT_AUTH_MODE 입니다: {settings.AI_AGENT_AUTH_MODE}"
        )
