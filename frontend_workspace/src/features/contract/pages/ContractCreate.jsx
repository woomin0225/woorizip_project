...
export default function ContractCreate() {
  ...
  const [form, setForm] = useState({ roomNo: '', moveInDate: '', termMonths: 12 });

  const onSubmit = async (e) => {
    e.preventDefault();
    try {
      await applyContract(form.roomNo, form);
  ...
  return (
    <section className={styles.page}><Container><Row className="justify-content-center"><Col lg="8"><Card className="shadow border-0"><CardBody>
      ...
      <form onSubmit={onSubmit}>
        ...
        <input className={styles.input} placeholder="입주일 (예: 2026-04-01)" value={form.moveInDate} onChange={(e) => setForm({ ...form, moveInDate: e.target.value })} />
        <input className={styles.input} placeholder="계약 개월수" value={form.termMonths} onChange={(e) => setForm({ ...form, termMonths: e.target.value })} />
