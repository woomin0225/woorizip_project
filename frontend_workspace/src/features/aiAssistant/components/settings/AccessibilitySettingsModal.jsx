...
export default function AccessibilitySettingsModal({ open, onClose }) {
  const { voiceModeEnabled, settings, enableVoiceMode, disableVoiceMode, updateSetting } = useVoiceMode();

  if (!open) return null;

  return (
    <div className={styles.overlay} role="dialog" aria-modal="true" aria-label="접근성 설정">
      <div className={styles.panel}>
        <div className={styles.header}>
          <div>
            <h2 className={styles.title}>접근성 설정</h2>
            <p className={styles.description}>일반 사용자와 시각장애인 사용자 모두를 위한 음성 지원과 화면 보기 설정입니다.</p>
          </div>
          <button type="button" className={styles.closeBtn} onClick={onClose} aria-label="접근성 설정 닫기">
            ×
          </button>
        </div>
        ...
        <section className={styles.section}>
          ...
          <label className={styles.checkboxRow}>
            <input
              ...
              onChange={(e) => updateSetting('autoReadPageSummary', e.target.checked)}
            />
            페이지 진입 시 자동 요약 읽기
          </label>
          <label className={styles.checkboxRow}>
            <input
              ...
              onChange={(e) => updateSetting('readFocusedElement', e.target.checked)}
            />
            현재 포커스 요소 읽기
          </label>
          <label className={styles.checkboxRow}>
            <input
              ...
              onChange={(e) => updateSetting('autoReadBotReplies', e.target.checked)}
            />
            우리봇 답변 자동 읽기
          </label>
          <label className={styles.checkboxRow}>
            <input
              ...
              onChange={(e) => updateSetting('voiceCommandEnabled', e.target.checked)}
            />
            음성 명령 사용
          </label>
        </section>
