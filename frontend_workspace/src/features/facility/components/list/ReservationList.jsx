...
export default function ReservationList() {
  ...
  if (error) return <div className={styles.facilityEmpty}>에러 발생: {error.message}</div>;
    ...
    <div className={styles.sectionBlock}>
          ...
          reservationList.map((r) => (
            ...
            >
              <div className={styles.oneLine}>
                ...
                {r.reservationDate} ({r.reservationStartTime.slice(0, 5)} ~ {r.reservationEndTime.slice(0, 5)})
                <span className={styles.textMuted}> | {r.reservationStatus}</span>
              </div>
      ...
      {pageResponse && pageResponse.totalPages > 1 && (
        <div className={styles.pager}>
          {Array.from({ length: pageResponse.totalPages }, (_, i) => i + 1).map((p) => (
            <button
              key={p}
              className={query.page === p ? `${styles.inlineBtn} ${styles.inlineBtnActive}` : styles.inlineBtn}
              onClick={() => setPage(p)}
            >
              {p}
            </button>
