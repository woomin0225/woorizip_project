...
async def lifespan(app: FastAPI):
    # 앱시작시 구동될 클라이언트 작성
    app.state.llmClient = QwenLlmClient(
...
async def summary_unified(req: SummaryReq):
    if req.target_type == "room":
        out = await summary.summarize_room(
            req.room_id,
            req.room or {},
        ...
        out = await summary.summarize_text(
            req.title, req.text or "", bullets=req.bullets
        )

    if req.include_tts:
        tts = await voice.speak(
            out.get("summary") or "", voice=req.voice, audio_format=req.audio_format
        )
...
async def listing_index(req: ListingIndexReq):
    return await listing.index_room(
        req.room_id,
        req.room,
        image_summaries=req.image_summaries,
...
async def listing_search(req: ListingSearchReq):
    return await listing.search_rooms(
        req.query, filters=req.filters, candidates=req.candidates, top_k=req.top_k
    )


@app.post("/ai/review/analyze", dependencies=[Depends(require_internal_api_key)])
async def review_analyze(req: ReviewAnalyzeReq):
    return await review.analyze_review(
        req.room_id, req.review_id, req.text, ingest=req.ingest, meta=req.meta
    )
