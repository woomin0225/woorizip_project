...
class Settings(BaseSettings):
    ...
    LLM_PROVIDER: str = Field(
        default="mock", description="mock | watsonx | gemini | groq"
    )
    ...
    OBJECT_DETECTION_PROVIDER: str = Field(
        default="mock", description="mock | groundingdino"
    )
    ...
    def validate(self) -> None:
        if self.LLM_PROVIDER.lower() == "watsonx":
            ...
            if missing:
                raise RuntimeError(
                    "LLM_PROVIDER=watsonx 이지만 환경변수 누락: " + ", ".join(missing)
                )
        ...
        if (
            self.OBJECT_DETECTION_PROVIDER.lower() == "groundingdino"
            and not self.GROUNDINGDINO_BASE_URL
        ):
            raise RuntimeError(
                "OBJECT_DETECTION_PROVIDER=groundingdino 이면 GROUNDINGDINO_BASE_URL 이 필요합니다."
            )

        if self.EMBEDDING_PROVIDER.lower() == "openai" and not self.OPENAI_API_KEY:
            raise RuntimeError(
                "EMBEDDING_PROVIDER=openai 이면 OPENAI_API_KEY 가 필요합니다."
            )

        if self.TOOL_EXECUTION_MODE.lower() == "execute" and not self.SPRING_BASE_URL:
            raise RuntimeError(
                "TOOL_EXECUTION_MODE=execute 이면 SPRING_BASE_URL 이 필요합니다."
            )
