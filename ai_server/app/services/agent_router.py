...
class AgentRouter:
    ...
    async def _classify_intent(self, instruction: str) -> str:
        ...
        # 안전장치
        allowed = {
            "RAG_QUERY","DOC_WRITE","SUMMARY","RECOMMEND","POLICY_CHECK",
            "MONITOR_VIEW_ABUSE","MONITOR_FACILITY",
            "LISTING_REGISTER","FACILITY_BOOKING","NAVIGATE_POST","LOCATION_PROSCONS","CHAT"
        }
        return intent if intent in allowed else "CHAT"

    async def run(self, user_id: str, instruction: str, extra: dict[str, Any] | None = None) -> dict:
        extra = extra or {}
        ...
        if intent == "RAG_QUERY":
            ...
            return {"intent": intent, "data": await self.rag.answer(instruction, top_k=top_k)}

        if intent == "DOC_WRITE":
            doc_type = extra.get("doc_type", "보고서")
            tone = extra.get("tone", "업무용")
            length = extra.get("length", "1~2 페이지")
            return {"intent": intent, "data": {"result": await self.doc.write(doc_type, instruction, tone=tone, length=length)}}

        if intent == "SUMMARY":
            title = extra.get("title")
            text = extra.get("text") or instruction
            bullets = int(extra.get("bullets", 5))
            return {"intent": intent, "data": await self.summary.summarize_text(title, text, bullets=bullets)}

        if intent == "RECOMMEND":
            candidates = extra.get("candidates", [])
            goal = extra.get("goal", "주거/매물 탐색")
            return {"intent": intent, "data": await self.reco.recommend(user_id, candidates, goal=goal)}
        ...
        if intent == "MONITOR_VIEW_ABUSE":
            ...
            return {"intent": intent, "data": await self.monitoring.analyze("view_abuse", payload, tone=extra.get("tone","업무용"))}

        if intent == "MONITOR_FACILITY":
            payload = extra.get("payload", {})
            return {"intent": intent, "data": await self.monitoring.analyze("facility_usage", payload, tone=extra.get("tone","업무용"))}
...
요청: {instruction}
"""
            return {"intent": intent, "data": await self.summary.summarize_text("location_proscons", text, bullets=int(extra.get("bullets", 6)))}
        ...
        if intent == "LISTING_REGISTER":
            ...
            if not extra.get("confirm"):
                ...
                draft = await self.tools.listing_register_draft(utterance=instruction, partial=partial)
                return {
                    "intent": intent,
                    "stage": "draft",
                    "draft": draft,
            ...
            return {
                "intent": intent,
                "stage": "confirm",
                "data": await self.tools.call("listing_register_confirm", {"draft": extra.get("draft"), "confirmed": True}),
            }

        if intent == "FACILITY_BOOKING":
            if not extra.get("confirm"):
                facilities = extra.get("facilities", [])
                draft = await self.tools.facility_booking_draft(utterance=instruction, facilities=facilities)
                return {"intent": intent, "stage": "draft", "draft": draft, "next": "사용자 확인(confirm=true) 후 실행"}

            return {
                "intent": intent,
                "stage": "confirm",
                "data": await self.tools.call("facility_booking_confirm", {"draft": extra.get("draft"), "confirmed": True}),
            }

        if intent == "NAVIGATE_POST":
            candidates = extra.get("candidates", [])
            plan = await self.tools.navigate_to_post(utterance=instruction, candidates=candidates)
